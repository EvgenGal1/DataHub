...
export class RolesController {
  ...
  createRole(@Body() createRoleDto: CreateRoleDto) {
    this.logger.debug(`req + Role: '${JSON.stringify(createRoleDto)}'`);
  ...
  findAllRoles() {
    this.logger.debug(`req << Roles All`);
  ...
  async findOneRole(@Param('id') id: number) {
    this.logger.debug(`req < Role.ID '${id}'`);
  ...
  async findRoleByValue(@Param('value') value: string) {
    this.logger.debug(`req <? Role Value '${value}'`);
  ...
  // restoreRole(@Param('id') id: number) {
